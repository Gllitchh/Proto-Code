...
void setup() {
  ...
  server.on("/move", HTTP_GET, []() {
    ...
    if (motor1.length() > 0) motor1Position = motor1.toInt();  // Store motor position
    if (motor2.length() > 0) motor2Position = motor2.toInt();  // Store motor position
  ...
  server.on("/getMotorPositions", HTTP_GET, []() {
    ...
    server.send(200, "text/plain", motorPositions);  // Send motor positions to the client
  });
